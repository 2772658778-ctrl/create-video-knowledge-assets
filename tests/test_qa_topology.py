...
def test_topology_derives_facets_from_actual_units(tmp_path: Path) -> None:
    asset = _complete_qa_asset(tmp_path)

    topology = build_qa_topology(asset)

    facets_by_id = {facet["unit_id"]: facet for facet in topology["facets"]}
    assert set(facets_by_id) == {"ku-concept", "ku-procedure"}
    assert "rel-procedure-concept" in facets_by_id["ku-concept"]["inbound_relation_ids"]
    assert "rel-procedure-concept" in facets_by_id["ku-procedure"]["outbound_relation_ids"]
    assert "ku-external" not in facets_by_id
...
def test_topology_refuses_a_symlinked_destination(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ValueError, match="topology destination must (not be a symlink|stay within the asset root)"
    ):
        build_qa_topology(asset)
