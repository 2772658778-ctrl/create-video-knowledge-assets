...
def render_course_tex(
    ...
    *,
    ...
    lines = [
        ...
        r"\DeclareCaptionFont{vkacaption}{\fontsize{9.5}{13}\selectfont}",
        r"\captionsetup{format=hang,width=.92\linewidth,font={vkacaption},labelfont={bf,sf},labelformat=vkagreen,labelsep=quad,justification=raggedright,singlelinecheck=false,skip=7pt}",
        ...
        r"\AtBeginDocument{\fontsize{11}{17.5}\selectfont}",
    ...
    if show_cover and one_sentence_summary:
        lines.extend(
            [
                ...
                r"\raggedright\fontsize{11}{17}\selectfont\color{vkaMuted}",
    ...
    if metadata_lines:
        lines.extend(
            [
                ...
                r"\centering\sffamily\fontsize{9}{13.5}\selectfont\color{vkaMuted}",
    ...
    if notes:
        lines.extend(
            [
                ...
                r"\noindent{\color{vkaRule}\rule{\linewidth}{0.4pt}}",
                r"\vspace{0.5\baselineskip}",
                r"{\fontsize{9}{13}\selectfont\color{vkaMuted} 正文中的上标编号对应下面列出的时间范围，标注该段内容在视频中的位置。\par}",
                r"\vspace{0.4\baselineskip}",
                r"{\fontsize{8.5}{12}\selectfont\ttfamily\color{black!65}",
                # Two columns keep every citation inside its own column; a
                # third column makes long entries wrap across the column break.
                r"\begin{multicols}{2}",
                r"\raggedcolumns",
