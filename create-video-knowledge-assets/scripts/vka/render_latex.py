...
def _render_block(block: Any) -> list[str]:
    ...
    kind = block.get("kind")
    if kind == "paragraph":
        paragraph = _escape_latex(_paragraph_text(block))
    ...
    if kind == "caption":
        return [rf"\textit{{{_escape_latex(_text_field(block, 'caption text'))}}}{_source_footnote(block)}"]
...
def _escape_table_cell(value: str) -> str:
    ...
    for match in URL_RE.finditer(value):
        ...
        parts.append(rf"\url{{{_escape_url(match.group())}}}")
        offset = match.end()
