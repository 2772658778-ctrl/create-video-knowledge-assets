...
class GroundedAnswer(BaseModel):
    ...
    knowledge_revision_sha256: str | None = None
    ...
    def validate_mode_and_scope(self) -> "GroundedAnswer":
        ...
        if self.answer_plan is not None:
            plan_revision = self.answer_plan.get("knowledge_revision")
            plan_sha256 = self.answer_plan.get("knowledge_revision_sha256")
            if (plan_revision is None) != (plan_sha256 is None):
                raise ValueError(
                    "answer_plan revision identity must include both id and sha256"
                )
            if plan_revision is not None:
                if (
                    self.knowledge_revision != plan_revision
                    or self.knowledge_revision_sha256 != plan_sha256
                ):
                    raise ValueError(
                        "knowledge_revision and knowledge_revision_sha256 must match answer_plan revision identity"
                    )
            elif self.knowledge_revision is not None:
                raise ValueError(
                    "knowledge_revision must be present in answer_plan when a revision answer is used"
                )
