...
Record each finished stage with
`vka complete-stage --asset "$ASSET_DIR" --stage <stage> --output <path>`
(repeat `--output` for every artifact of that stage). The command writes the
stage record and the SHA-256 of each output into `manifest.json`; hand-editing
the manifest or hashing outputs in a one-off script is not allowed. When an
artifact changes after its stage was recorded, re-run `complete-stage` for that
stage and then `vka verify-asset`.
