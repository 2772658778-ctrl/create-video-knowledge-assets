...
interpretations derived from source evidence. Every unit must remain traceable to
...
- `knowledge/outline.json`: the profile-specific authoring outline. A
  general-deep view uses the contract in `profile-general-deep.md`; a
  course-notes view keeps using `knowledge/teaching_outline.json` as its only
  mandatory teaching bridge.
