...
8. For `general-deep` and `course-notes`, generate and validate one renderer-neutral document, then render it. PDF is the delivered format; Markdown and HTML come from the same document and exist for review, not as separate products. Compile and inspect the PDF before handing it over. Publish a shareable copy with `vka package-demo --asset <asset> --profile <profile>`: it copies the cover and the cited inspected frames next to the document and rewrites their paths. Every profile's PDF carries a cover with the title, theme, one-sentence core summary, and source metadata such as video title, author/uploader, publish date, and subtitle/transcript source; the table of contents opens the first content page, and the numbered source times are listed in a 来源时间 block on the closing page. Keep structured provenance in JSON for all formats. Do not render or deliver one-sentence chapter summaries, unresolved `???` placeholders, unreviewed ASR, uninspected visual claims, or notes that fail to teach the video's main idea.
