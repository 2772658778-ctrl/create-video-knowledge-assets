...
description: Create evidence-backed Chinese video summaries and knowledge assets from Bilibili URLs or local knowledge videos. Use when Codex needs to produce a deep video summary or course notes with PDF as the primary deliverable and optional HTML or Markdown from the same document view; also use for explicitly requested creator articles, enterprise knowledge records, research briefs, or grounded Q&A over a completed course-notes asset.
---

# Create Video Knowledge Assets

Use this workflow for a Bilibili URL or a local video containing knowledge content. Keep the work evidence-first: every knowledge claim must trace back to transcript windows or checked visual frames. Treat deep summaries and course notes as the primary release scope, with PDF as the primary reader deliverable. HTML and Markdown must come from the same renderer-neutral document rather than separate rewrites.
...
- Use `creator-article`, `enterprise-knowledge`, and `research-brief` only when the user explicitly requests that business view. Read `references/profile-routing.md`, `references/p3-profile-workflow.md`, `references/p3-profile-authoring.md`, `references/projection-plan.md`, the selected profile reference, and `references/document-schema.md` before authoring it.

## Required Workflow

1. Run preflight before any extraction or generation. Check the tools required by the selected path: Bilibili acquisition needs `yt-dlp`; local-video inspection needs `ffprobe`; frame and audio extraction need `ffmpeg`; missing or unreliable CC needs `whisper`; PDF compilation needs `xelatex`. If a required tool is missing, stop or degrade only according to `references/p1-workflow.md`, and report the missing items.
...
3. Prefer reliable human CC subtitles. If no reliable CC exists, run Whisper ASR. Preserve the original SRT, normalize it into `timeline.raw.jsonl`, repair ASR errors with the prompt workflow in `references/transcript-repair.md`, and only then write downstream `timeline.jsonl`.
4. Create an asset directory for the run. Store raw inputs and downloads under `source/`; store `timeline.jsonl`, frame manifests, and grounding records under `evidence/`.
...
7. Build the reader-facing plan from the evidence and knowledge layers. For the default `general-deep` profile, create the outline required by `references/profile-general-deep.md`. For an explicit `course-notes` request, generate `knowledge/teaching_outline.json` and validate it with `vka validate-teaching-outline`. Do not project `knowledge/units.jsonl` directly into any reader-facing format.
8. For `general-deep` and `course-notes`, generate and validate one renderer-neutral document, then render the requested Markdown, HTML, and LaTeX outputs from it. Compile and inspect PDF as the primary deliverable unless the user explicitly requests a non-PDF-only result. For `course-notes`, the title page must include a concise theme, a one-sentence core summary, and source metadata such as video title, author/uploader, publish date, and subtitle/transcript source. PDF output should show compact source-time footnotes at the bottom of the page; keep structured provenance in JSON for all formats. Do not render or deliver one-sentence chapter summaries, unresolved `???` placeholders, unreviewed ASR, uninspected visual claims, or notes that fail to teach the video's main idea.
9. Before R3 profile projection, run `vka verify-asset --asset <asset>`. Build, validate, and persist the selected profile's `projection-plan.json` before drafting its supplied authored document. When a directly inspected usable frame exists, include it as a grounded `image` block; do not replace it with a textual description. R3 may write `views/<profile_id>` and `outputs/<profile_id>` only; it must not change `source/`, `evidence/`, or `knowledge/`. Verify canonical hashes before and after projection.

## Grounded Q&A

Treat Q&A as a secondary workflow and use it only when the user explicitly asks a question about a completed course-notes asset. Select `video_only` unless the user asks for context or exploration. Read `references/p4-qa.md`, `references/p4-video-topology.md`, `references/p4-qa-authoring.md`, and `references/p4-qa-evaluation.md`; read `references/p4-external-evidence.md` and `references/p4-answer-presentation.md` only for a context mode. Run `vka verify-asset`, `vka build-qa-index`, `vka build-qa-topology`, `vka retrieve-qa`, and `vka plan-qa` in order; author from the selected references, then run `vka validate-answer`. Retrieval and planning never generate an answer. Video claims must cite only video evidence. Context modes require separately registered `external:` evidence and visibly separate non-video blocks.
...
- Read `references/p4-qa.md` before building a Q&A index, retrieving evidence, authoring a grounded answer, or validating it.
- Read `references/p4-video-topology.md` before building or interpreting Q&A topology.
- Read `references/p4-qa-authoring.md` before turning a retrieval pack and answer plan into an authored answer.
- Read `references/p4-qa-evaluation.md` before evaluating Q&A coverage, readability, or limits.
- Read `references/p4-external-evidence.md` before registering, validating, or citing P4.2 external evidence.
- Read `references/p4-answer-presentation.md` before authoring a context, exploration, conflict, or model-background Q&A section.
