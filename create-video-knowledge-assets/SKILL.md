...
- Read `references/pdf-typography.md` before changing a renderer, judging a rendered PDF, or deciding how a document is delivered. It fixes the type scale, the single accent colour, figure and contents handling, and the rendering traps that already cost us iterations.
