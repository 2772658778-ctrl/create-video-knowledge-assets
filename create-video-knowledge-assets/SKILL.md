...
3. Prefer reliable human CC subtitles. If no reliable CC exists, run Whisper ASR. Preserve the original SRT, normalize it into `timeline.raw.jsonl` with `--id-prefix raw-`, repair ASR errors with the prompt workflow in `references/transcript-repair.md`, and only then write downstream `timeline.jsonl` with `--parent-prefix raw-`. Read `vka timeline-digest` output when you build knowledge units instead of re-reading the full JSONL.
